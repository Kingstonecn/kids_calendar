...
class _DualCalendarState extends State<DualCalendar> {
  ...
  Widget _buildCalendarGrid(
    ...
    for (int i = 0; i < grid.length; i += 7) {
      weeks.add(grid.sublist(i, i + 7));
    }
    weeks.removeWhere((w) => w.every((d) => d == null || d.month != _currentMonth.month));

    // 根据屏幕尺寸计算单元格高度（平板增加约10%）
    final screenSize = MediaQuery.of(context).size;
    final isTablet = screenSize.width >= 600;
    final cellHeight = (screenSize.height * (isTablet ? 0.1235 : 0.095)).clamp(66.0, 170.0);
    ...
    Widget buildCell(DateTime date) {
      ...
      // 获取农历
      final lunar = LunarCalendar.simpleLunar(date.year, date.month, date.day);
      // 优先显示法定假日, 不显示农历
      final legalHoliday = LunarCalendar.getLegalHoliday(date.year, date.month, date.day);
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          padding: const EdgeInsets.only(top: 2, left: 1, right: 1, bottom: 2),
          decoration: BoxDecoration(
          ...
          return SizedBox(
            ...
            child: Row(
              children: List.generate(7, (di) {
                ...
                return Expanded(child: buildCell(date));
              }),
            ),
          );
        }),
